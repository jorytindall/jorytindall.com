...
import { Input, ItemWrapper, Label, TextArea } from 'components/form'
    ...
    const getFormControls = input.map(field => {
        switch (field._type) {
            ...
            case 'textarea': return (
                <ItemWrapper key={field._key}>
                    <Label>{field.label}</Label>
                    <TextArea placeholder={field.placeholder} name={field.name} rows={5} />
                </ItemWrapper>
            );
            case 'submit': return (
                <Button key={field._key} type='submit' variant='primary' size='default'>{field.text}</Button>
            )
        }
    })
