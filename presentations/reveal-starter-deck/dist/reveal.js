...
!(function (e, t) {
	...
	class k {
		...
		getUnmatchedAutoAnimateElements(e) {
			return [].slice.call(e.children).reduce((e, t) => {
				...
				return (
					t.hasAttribute('data-auto-animate-target') ||
						i ||
						e.push(t),
	...
	class L {
		...
		goto(e, t = 0) {
			...
			if (i && this.Reveal.getConfig().fragments) {
				let s = this.sort(
					i.querySelectorAll('.fragment:not(.disabled)'),
				);
				if (s.length) {
					if ('number' != typeof e) {
						let t = this.sort(
		...
		onDocumentKeyDown(e) {
			...
			let n =
				...
				r =
					!(
						(-1 !==
							[32, 37, 38, 39, 40, 78, 80].indexOf(e.keyCode) &&
							e.shiftKey) ||
						e.altKey
					) &&
	...
	function K(a, r) {
		...
		function Ie(e, i) {
			t($.slides, 'section > .stretch, section > .r-stretch').forEach(
				(t) => {
					let s = ((e, t = 0) => {
						if (e) {
							let i,
					...
					if (/(img|video)/gi.test(t.nodeName)) {
						...
						(t.style.width = i * a + 'px'),
							(t.style.height = n * a + 'px');
					} else
						(t.style.width = e + 'px'), (t.style.height = s + 'px');
				},
			);
		}
		function Te(e, t) {
			...
			return (
				...
				'string' == typeof n.width &&
					/%$/.test(n.width) &&
					(n.width =
						(parseInt(n.width, 10) / 100) * n.presentationWidth),
		...
		function Ke(e, i, s, n) {
			...
			d &&
				...
				(!d && g) ||
					(Q.stopEmbeddedContent(g), Q.startEmbeddedContent(f)),
				requestAnimationFrame(() => {
					me(fe(f));
				}),
				le.update(),
		...
		function $e(e = tt()) {
			e.forEach((t, i) => {
				...
				s.parentNode === t.parentNode &&
					t.parentNode.insertBefore(t, s);
		...
		function Ze(e) {
			...
			if (e) {
				...
				(s = Math.max(o.indexOf(a), 0)),
					...
					i &&
						(n = Math.max(
							t(e.parentNode, 'section').indexOf(e),
							0,
						));
			}
			if (!e && f) {
		...
		function ot(e, t) {
			...
			return s && s.length && 'number' == typeof t
				? s
					? s[t]
					: void 0
				: i;
			...
			if (e || !1 === se.prev())
				if (Ge().up) vt({ skipFragments: e });
					...
					if (
						...
						let e =
							i.querySelectorAll('section').length - 1 || void 0;
						Ke(c - 1, e);
		...
		function bt(e) {
			...
			if (
				...
				'}' === t.charAt(t.length - 1) &&
				((t = JSON.parse(t)),
				t.method && 'function' == typeof d[t.method])
			)
