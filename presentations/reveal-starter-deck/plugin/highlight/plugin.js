...
const Plugin = {
	...
	init: function (reveal) {
		...
		Array.from(
			...
			let substitute = block.querySelector(
		...
		if (block.hasAttribute('data-line-numbers')) {
			...
			if (slide) {
				var scrollFirstHighlightIntoView = function () {
					Plugin.scrollHighlightedLineIntoView(
						block,
						scrollState,
						true,
					);
	 ...
	 */
	scrollHighlightedLineIntoView: function (
		block,
		scrollState,
		skipAnimation,
	) {
		...
		viewportHeight -=
			parseInt(blockStyles.paddingTop) +
			parseInt(blockStyles.paddingBottom);
		...
		if (skipAnimation === true || startTop === targetTop) {
			...
			var animate = function () {
				...
				block.scrollTop =
					startTop +
					(targetTop - startTop) * Plugin.easeInOutQuart(time);
