...
(w.punctuation = u(w.punctuation, 'u')
	...
	(w._escapes = /\\([punct])/g),
	(w._comment = u(b._comment).replace('(?:--\x3e|$)', '--\x3e').getRegex()),
...
class y {
	...
	constructor(e) {
		...
		this.options.pedantic
			...
			: this.options.gfm &&
			  ((n.block = b.gfm),
			  this.options.breaks ? (n.inline = w.breaks) : (n.inline = w.gfm)),
	...
	inlineTokens(e, t = []) {
		...
		for (
			;
			null != (i = this.tokenizer.rules.inline.anyPunctuation.exec(o));
			...
			switch (r.type) {
				...
				case 'heading': {
					...
					n += this.renderer.heading(
						...
						e.depth,
						this.parseInline(e.tokens, this.textRenderer).replace(
							h,
							(e, t) =>
		...
		return (
			e.forEach((e) => {
				...
				if (
					...
					const t = this.defaults.renderer || new _(this.defaults);
