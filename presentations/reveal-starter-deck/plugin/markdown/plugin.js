...
const Plugin = () => {
	...
	function getSlidifyOptions(options) {
		...
		options.notesSeparator =
			options.notesSeparator || DEFAULT_NOTES_SEPARATOR;
	...
	function createMarkdownSlide(content, options) {
		...
		var notesMatch = content.split(
			new RegExp(options.notesSeparator, 'mgi'),
		);
			...
			for (var i = 0; i < element.childNodes.length; i++) {
				...
				if (i > 0) {
					...
					while (j >= 0) {
						...
						if (
							typeof aPreviousChildElement.setAttribute ==
								'function' &&
		...
		[].slice.call(sections).forEach(function (section) {
			...
			addAttributes(
				...
				section.getAttribute('data-element-attributes') ||
					section.parentNode.getAttribute(
				...
				renderer.code = (code, language) => {
					...
					if (CODE_LINE_NUMBER_REGEX.test(language)) {
						...
						lineNumbers = language
							.match(CODE_LINE_NUMBER_REGEX)[3]
							.trim();
						lineNumbers = `data-line-numbers="${lineNumbers}"`;
						language = language
							.replace(CODE_LINE_NUMBER_REGEX, '')
							.trim();
					}
