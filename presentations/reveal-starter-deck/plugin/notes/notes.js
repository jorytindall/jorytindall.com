			...
			if (e) {
				...
				if (t.header.length === t.align.length) {
					...
					for (r = t.header.length, n = 0; n < r; n++)
						t.header[n].tokens = this.lexer.inline(
							t.header[n].text,
						);
	...
	class y {
		...
		inlineTokens(t, e = []) {
			...
			for (
				;
				null !=
				(r = this.tokenizer.rules.inline.anyPunctuation.exec(l));
			...
			return (
				t.forEach((t) => {
					...
					if (
						...
						const e =
							this.defaults.renderer || new v(this.defaults);
					...
					if (t.tokenizer) {
						const e =
							this.defaults.tokenizer || new b(this.defaults);
