...
export default class AutoAnimate {
	...
	autoAnimateElements(from, to, elementOptions, animationOptions, id) {
		...
		if (
			...
			if (translate || scale) {
				...
				if (translate)
					transform.push(`translate(${delta.x}px, ${delta.y}px)`);
				if (scale)
					transform.push(`scale(${delta.scaleX}, ${delta.scaleY})`);
	...
	getLocalBoundingBox(element) {
		...
		return {
			...
			width:
				Math.round(element.offsetWidth * presentationScale * 100) / 100,
			height:
				Math.round(element.offsetHeight * presentationScale * 100) /
				100,
		};
	}
		...
		return [].slice.call(rootElement.children).reduce((result, element) => {
			...
			if (element.querySelector('[data-auto-animate-target]')) {
				result = result.concat(
					this.getUnmatchedAutoAnimateElements(element),
				);
			}
