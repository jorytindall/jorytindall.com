...
export default function (revealElement, options) {
	...
	function setViewport() {
		...
		if (config.embedded === true) {
			dom.viewport =
				Util.closest(revealElement, '.reveal-viewport') ||
				revealElement;
		}
	...
	function configure(options) {
		...
		const numberOfSlides =
			dom.wrapper.querySelectorAll(SLIDES_SELECTOR).length;
	...
	function enablePreviewLinks(selector = 'a') {
		Array.from(dom.wrapper.querySelectorAll(selector)).forEach(
			(element) => {
	...
	function getComputedSlideSize(presentationWidth, presentationHeight) {
		...
		if (typeof size.width === 'string' && /%$/.test(size.width)) {
			size.width =
				(parseInt(size.width, 10) / 100) * size.presentationWidth;
		}

		// Slide height may be a percentage of available height
		if (typeof size.height === 'string' && /%$/.test(size.height)) {
			size.height =
				(parseInt(size.height, 10) / 100) * size.presentationHeight;
		}
	...
	function resetVerticalSlides() {
		getHorizontalSlides().forEach((horizontalSlide) => {
			Util.queryAll(horizontalSlide, 'section').forEach(
				(verticalSlide, y) => {
					if (y > 0) {
	...
	function getProgress() {
		...
		if (currentSlide) {
			...
			if (allFragments.length > 0) {
				...
				pastCount +=
					(visibleFragments.length / allFragments.length) *
					fragmentWeight;
			}
		}
		...
		if (currentSlide && config.autoSlide !== false) {
			...
			if (fragmentAutoSlide) {
				...
				if (currentSlide.querySelectorAll('.fragment').length === 0) {
					Util.queryAll(currentSlide, 'video, audio').forEach(
						(el) => {
							if (el.hasAttribute('data-autoplay')) {
								if (
									autoSlide &&
									(el.duration * 1000) / el.playbackRate >
										autoSlide
								) {
									autoSlide =
										(el.duration * 1000) / el.playbackRate +
		...
		if (skipFragments || fragments.prev() === false) {
			if (availableRoutes().up) {
				...
				if (
					...
					let v =
						previousSlide.querySelectorAll('section').length - 1 ||
						undefined;
					let h = indexh - 1;
					slide(h, v);
				} else {
