			...
			return rollup({
				...
				plugins: [
					...
					babel({
						...babelConfig,
						ignore: [
							/node_modules\/(?!(highlight\.js|marked)\/).*/,
						],
					}),
					terser(),
				],
				...
				qunit
					...
					.then((result) => {
						if (result.stats.failed > 0) {
							console.log(
								`${'!'} ${filename} [${result.stats.passed}/${result.stats.total}] in ${result.stats.runtime}ms`
									.red,
							);
		...
		tests
			.then(() => {
				if (failingTests > 0) {
					reject(
						new Error(
							`${failingTests}/${totalTests} tests failed`.red,
						),
					);
				} else {
					console.log(
						`${'✔'} Passed ${totalTests} tests`.green.bold,
					);
					resolve();
				}
			})
